...
HTTP isteklerinde session reuse + 10dk TTL ile cookie tazeleme uygular.
"""
...
PORTFOY_KOD_LABEL: Dict[str, str] = {
    ...
    "kh":    "Katılım Hesabı",
    "khau":  "Altın Katılım Hesabı",
    "khd":   "Döviz Katılım Hesabı",
    "khtl":  "TL Katılım Hesabı",
...
def _get_session() -> requests.Session:
    ...
    if _session is None or now - _session_init_time > _SESSION_TTL:
        ...
        # Anti-bot warmup: API kullanılan bir sayfayı ziyaret ederek
        # ekstra session cookie alınır — bu olmadan ilk POST boş yanıt döner.
        try:
            s.get(
...
def fonlar_gunluk_detay_hepsi(fon_tipi: str = "YAT",
                                bas_tarih: Optional[str] = None,
                                bit_tarih: Optional[str] = None,
                                sayfa_boyutu: int = 250) -> List[Dict[str, Any]]:
...
def fonlar_donemsel_getiri(fon_tipi: str = "YAT",
    ...
    kurucu: kurucuKodu ile filtreleme (None=tümü).
    """
...
def fonlar_son_fiyat_bulk(fon_tipi: str = "YAT", tarih: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
    """BULK son fiyat — tüm fonlar için fiyat eşlemesi.
